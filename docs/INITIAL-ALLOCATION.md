...
5. After successful block inclusion under the default confirmation policy, WORM verifies factory, router, supply-mint and purchase-transfer events against the saved intent. Only then does it record the token and submit exactly 1% of launch supply to the saved creator wallet.
6. After the creator transfer succeeds in a block and its event matches, the allocation is complete. WORM retains 1% plus the small rounding remainder from USDG’s six-decimal precision. It does not buy again to rebalance those percentages later.
...
Approval confirmation, funding checks and chain inclusion take time. At the chosen timestamp the server begins the workflow; the countdown does not guarantee instant token creation. By default, verified successful inclusion lets the workflow continue immediately without waiting for finality. The optional WH_TX_CONFIRMATION=finalized policy waits for the stronger settlement boundary. See PAYMENT-RECOVERY.md for reorganization monitoring and the tradeoff.
