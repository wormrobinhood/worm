...
def test_demo_treasury_never_counts():
    r = R.compute({"scorecard": {}}, _lab(30, 0.2, stdev=0.1), RUNWAY_OK, treasury_is_demo=True)
...
def test_real_money_and_full_evidence_is_ready(monkeypatch):
    ...
    r = R.compute({"scorecard": card}, _lab(40, 0.15, stdev=0.3), RUNWAY_OK, treasury_is_demo=False)   # lower bound +5.5%
...
def test_losing_best_rule_blocks_ready_even_at_high_score():
    ...
    r = R.compute({"scorecard": card}, _lab(40, -0.05, stdev=0.2), RUNWAY_OK, treasury_is_demo=False)
...
def test_avoid_everything_earns_no_skill():
    ...
    for rugged in (10, 14, 18):
        ...
        acc = part(R.compute({"scorecard": card}, _lab(0, None), RUNWAY_OK, False), "accuracy")
...
def test_lab_is_judged_by_the_lower_confidence_bound(monkeypatch):
    ...
    r = R.compute({"scorecard": {}}, _lab(10, 0.10, stdev=1.0), RUNWAY_OK, False)      # +10% mean, wild spread
    p = part(r, "lab")
    assert p["positive"] is False and p["score"] < 50 and abs(p["lcb"] - (0.10 - 2.0 / 10 ** 0.5)) < 1e-4
    assert "not proven" in p["detail"]
    p = part(R.compute({"scorecard": {}}, _lab(10, 0.10, stdev=1.0, lcb=0.05), RUNWAY_OK, False), "lab")   # the lab's own bound wins
    assert p["positive"] is True and p["lcb"] == 0.05 and p["score"] == 42
    p = part(R.compute({"scorecard": {}}, _lab(10, 0.10), RUNWAY_OK, False), "lab")     # no spread known: not proven
    assert p["positive"] is False and p["lcb"] is None and p["score"] == 17 and "no confidence bound" in p["detail"]
    monkeypatch.setattr(lab, "LAB_MIN_N", 30)
    p = part(R.compute({"scorecard": {}}, _lab(10, 0.10, stdev=0.01), RUNWAY_OK, False), "lab")
...
def test_score_is_bounded():
    ...
    r = R.compute({"scorecard": card}, _lab(1000, 9.9, stdev=0.1), {**RUNWAY_OK, "runway_days_no_income": 9e6, "surplus_usd": 9e6}, False)
