...
function placeOverview(v){const band=document.querySelector('#liveband'),grid=document.querySelector('#grid'),feed=document.querySelector('[data-panel=feed]'),voice=document.querySelector('[data-panel=voice]');if(!band||!grid||!feed||!voice)return;
  if(v==='overview'){if(feed.parentElement!==band)band.appendChild(feed);if(voice.parentElement!==band)band.appendChild(voice)}
  else{if(feed.parentElement!==grid)grid.insertBefore(feed,grid.firstElementChild);if(voice.parentElement!==grid)grid.appendChild(voice)}}
...
function enhance(s){if(s.links&&s.links.x){xlink.href=s.links.x;xlink.hidden=false}if(s.stats?.last_block!=null&&s.stats.last_block!==window.observedBlock){window.observedBlock=s.stats.last_block;window.blockAdvancedAt=Date.now()}const st=s.stats||{},rw=s.runway||{},rd=s.readiness||{},sc=s.scout||{},ch=s.character||{};document.querySelector('#specimen-block').innerHTML='ROBINHOOD CHAIN<br><strong>BLOCK '+esc(st.last_block??'…')+'</strong>';document.querySelector('#mode-label').textContent=s.live?'Live execution':'Not live · no transactions';const metrics=[['Tokens screened',num(st.scored),'Across the indexed window'],['Warnings confirmed',sc.called??'…',`${sc.checked_warnings??0} assessed warnings checked`],['Actual treasury',presentMoney(ch.usd_real??rw.treasury_usd),'Spendable balance, gold reserve excluded'],['Trading readiness',`${rd.score??0}<em> / 100</em>`,(s.trader&&s.trader.enabled===false)?`Trading off by policy · threshold ${rd.ready_at??80}`:rd.ready?'Evidence threshold met':`Evidence threshold: ${rd.ready_at??80}`]];document.querySelector('#overview-metrics').innerHTML=metrics.map(([l,v,d])=>`<div class="metric"><span>${l}</span><strong>${v}</strong><small>${d}</small></div>`).join('');document.querySelector('#grow').textContent='The worm grows with the treasury it holds.';
if(ch.demo){const stage=document.querySelector('#stage');stage.innerHTML=`${s.live?'':'<span class="demo">NOT LIVE</span> '}stage ${(ch.stage||0)+1}/${num(ch.stages||7)} · ${esc(ch.stage_name||'hatchling')} · character balance ${presentMoney(ch.usd)} <span class="meta">(simulated)</span>${ch.next_usd?` · next size at ${presentMoney(ch.next_usd)}`:' · fully grown'}`}
const titles={feed:'Recent token scans',wallet:'Wallet & fee income',trades:'Trading activity',runway:'Operating runway',brain:'Rule performance',lab:'Strategy comparisons',paper:'Paper portfolio',advisor:'Advisor',bad:'Creator signals',voice:'Field notes'};for(const [k,v] of Object.entries(titles)){const el=document.querySelector(`[data-panel="${k}"] .ttl`);if(el)el.textContent=v}
