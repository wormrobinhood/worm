...
def check(rpc, db, hub, now=None):
    ...
    try:
        ...
        if outbox.pending():add('transaction_unresolved','A transaction needs reconciliation. Keep one signer and inspect its private journal.')
        if db.one("SELECT 1 FROM ledger WHERE kind LIKE '%_pending' AND ts<?", (now-600,)):
            add('payment_unresolved','A payment has not settled within 10 minutes. Review its chain/provider evidence.')
        if db.one("SELECT 1 FROM fee_sweeps WHERE state='pending' AND ts<?", (now-600,)) or db.one("SELECT 1 FROM gas_refills WHERE state='pending' AND ts<?", (now-600,)):
        ...
        if heartbeat and now-float(heartbeat)>max(900,C.MARK_EVERY_S*3):
